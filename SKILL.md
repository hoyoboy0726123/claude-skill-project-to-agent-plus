...
| **Phase 3 選訂閱 → 必先跑 3b 進場門(亮差異+確認)才動工** | 沒做 → 使用者不知道幻覺偵測失效/codex shell 不受 ACL 約束就上路,資安與期待雙翻車 |
...
選了任一個 → **先跑 Phase 3b 的「⛔ 進場門」**:亮出與 API 路線的取捨(幻覺偵測替代、檔案偵測改掃文字、codex 原生 shell 不受 ACL 約束、沙盒限制、額度上限、多人隔離註記),AskUserQuestion 確認才動工;不確認 → 回原流程零損失。確認後跑 **Phase 3b**(`references/phase3b-subscription-cli.md`:MCP server 曝露工具 + `cli_brain.py` 驅動 + session 管理 + 全部參數地雷)。**沒選 → Phase 3b 標 deleted,一切照原有 API-key 流程,零改動**。兩條路可並存(settings 切換)。注意:訂閱路線的大腦是 CLI 自帶的 agentic loop,Phase 4 的 orchestrator 只服務 API-key 路線;Phase 6 的 hallucination 偵測在訂閱路線由「交付契約+系統偵測網」替代(Phase 3b 文件 §7)。
